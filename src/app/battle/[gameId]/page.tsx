...
function GameRoomContent() {
    ...
    // Auto-select random cards for DRAFT mode
    const draftAutoSelectDone = useRef(false);
    useEffect(() => {
        // Only for draft mode, when selecting, and not already ready
        if (
            game?.gameMode === 'draft' &&
            game?.status === 'selecting' &&
            !myPlayer?.ready &&
            user &&
            inventoryCards.length >= GAME_CONSTANTS.CARDS_PER_GAME &&
            !draftAutoSelectDone.current
        ) {
            draftAutoSelectDone.current = true;

            // Shuffle and pick 7 random cards from inventory
            const shuffled = [...inventoryCards].sort(() => Math.random() - 0.5);
            const randomCards = shuffled.slice(0, GAME_CONSTANTS.CARDS_PER_GAME).map(c => c.id);

            // Auto-submit these cards
            selectCards(gameId, user.uid, randomCards).then(result => {
                if (!result.success) {
                    console.error('Draft auto-select failed:', result.error);
                    setError('Failed to auto-select cards for draft mode');
                    draftAutoSelectDone.current = false; // Allow retry
                }
            });
        }
    }, [game, myPlayer, user, inventoryCards, gameId]);
            ...
            <div className="max-w-6xl mx-auto">
                ...
                {game.status === 'selecting' && (
                    <SelectionPhase
                        ...
                        error={error}
                        isDraftMode={game.gameMode === 'draft'}
                    />
                )}
...
function SelectionPhase({
    ...
    // Draft mode: show auto-selecting UI
    if (isDraftMode && !isReady) {
        return (
            <div className="text-center py-20">
                <div className="text-8xl mb-8 animate-spin-slow">🎲</div>
                <h2 className="text-3xl font-bold text-white mb-4">Draft Mode</h2>
                <p className="text-gray-400 text-lg mb-4">Auto-selecting 7 random cards...</p>
                <div className="h-2 w-64 mx-auto bg-gray-700 rounded-full overflow-hidden">
